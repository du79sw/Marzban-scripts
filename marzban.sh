...
perform_db_migration() {
    colorized_echo blue "Performing database migration..."
    $COMPOSE -f $COMPOSE_FILE -p "$APP_NAME" exec marzban alembic upgrade head
    if [ $? -eq 0 ]; then
        colorized_echo green "Database migration completed successfully."
    else
        colorized_echo red "Database migration failed!"
        exit 1
    fi
}
...
FILES_URL_PREFIX="https://raw.githubusercontent.com/du79sw/Marzban/master"
    # Parse options
   while [[ $# -gt 0 ]]; do
        key="$1"
        case $key in
...
perform_db_migration
    ...
    colorized_echo yellow "IMPORTANT: После внесения изменений нужно будет выполнить миграцию базы данных для добавления нового поля."
    colorized_echo yellow "To perform migration after changes, run: marzban restart && marzban cli migrate"
